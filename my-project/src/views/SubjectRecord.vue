...
// 💡 1. 초기 데이터 로드 (전교생 DB & 내 과목 리스트)
onMounted(async () => {
  // 로컬 스토리지에서 과목 리스트 불러오기 (기본값 세팅)
...
// 과목이나 학생이 바뀌면 기록 다시 불러오기
watch([selectedStudent, selectedSubject], fetchRecords)
...
<template>
  <div class="max-w-7xl mx-auto p-4 sm:p-6 font-sans">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 border-b pb-4">
      <div>
        <h2 class="text-2xl sm:text-3xl font-black text-gray-800 tracking-tight">📝 교과 수업 기록</h2>
        <p class="text-gray-500 mt-1 font-bold">과목별 학생들의 수업 태도와 활동(세특)을 기록합니다.</p>
      </div>
      
      <div class="flex items-center gap-2 bg-white p-2 border border-gray-200 rounded-xl shadow-sm">
        <span class="font-bold text-gray-700 text-sm pl-2">과목:</span>
        ...
        <div v-if="isAddingSubject" class="flex gap-1 ml-2">
          ...
          <button @click="isAddingSubject = false" class="px-2 py-1.5 bg-gray-200 text-gray-700 font-bold rounded text-sm hover:bg-gray-300">취소</button>
        </div>
        <div v-else class="flex gap-1 ml-2">
          <button @click="isAddingSubject = true" class="px-3 py-2 bg-gray-100 text-gray-700 font-bold rounded-lg text-sm hover:bg-gray-200 transition-colors">➕ 과목 추가</button>
          <button v-if="selectedSubject" @click="deleteSubject(selectedSubject)" class="px-3 py-2 text-red-500 hover:bg-red-50 font-bold rounded-lg text-sm transition-colors" title="현재 과목 삭제">🗑️</button>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col lg:flex-row gap-6">
      ...
      <div class="w-full lg:w-1/3 bg-white rounded-2xl shadow-sm border border-gray-200 flex flex-col h-[75vh]">
        <div class="p-4 border-b border-gray-200 bg-gray-50 rounded-t-2xl">
          ...
          <p class="text-xs text-blue-600 font-bold">조회된 학생: {{ filteredStudents.length }}명</p>
        </div>

        <div class="overflow-y-auto flex-1 p-2 custom-scrollbar">
          <div v-if="filteredStudents.length === 0" class="text-center py-10 text-gray-400 font-medium text-sm">
            학생 데이터가 없습니다.<br>전체 학생 관리에서 먼저 등록해주세요.
          </div>
          <button 
          ...
          >
            <span class="font-bold" :class="selectedStudent?.id === student.id ? 'text-blue-800' : 'text-gray-800'">
      ...
      <div class="w-full lg:w-2/3 flex flex-col h-[75vh]">
        <div v-if="!selectedStudent" class="flex-1 flex flex-col items-center justify-center bg-white rounded-2xl shadow-sm border border-gray-200 border-dashed text-gray-400 p-6">
          <span class="text-5xl mb-4">👈</span>
          <p class="text-lg font-bold text-gray-600">왼쪽 명단에서 학생을 선택해주세요.</p>
          <p class="text-sm">선택한 학생의 <b>[{{ selectedSubject || '과목' }}]</b> 활동 기록을 작성할 수 있습니다.</p>
        </div>

        <div v-else class="flex-1 flex flex-col gap-4">
          
          <div class="bg-blue-50 rounded-2xl shadow-sm border border-blue-100 p-5 shrink-0">
            ...
            <div class="flex flex-col gap-3">
              <input v-model="recordDate" type="date" class="w-40 p-2 border border-blue-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 font-bold text-gray-900 bg-white">
              <textarea 
                v-model="recordContent" 
                class="w-full h-28 p-3 border border-blue-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 resize-none text-gray-900 bg-white font-medium placeholder-gray-400"
                placeholder="수업 중 관찰한 학생의 특징, 참여도, 세특 사항을 구체적으로 적어주세요."
              ></textarea>
              <div class="flex justify-end">
                <button @click="saveRecord" :disabled="isSaving" class="px-6 py-2.5 bg-blue-600 text-white font-bold rounded-xl shadow-md hover:bg-blue-700 transition-colors disabled:opacity-50">
                  {{ isSaving ? '저장 중...' : '💾 기록 저장' }}
                </button>
              </div>
            </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-200 flex-1 p-5 overflow-y-auto custom-scrollbar">
            <h4 class="font-bold text-gray-800 mb-4 flex items-center gap-2">
              <span>📋 누적 기록</span>
              <span class="bg-gray-100 text-gray-600 px-2 py-0.5 rounded text-xs">{{ records.length }}건</span>
            </h4>
            
            <div v-if="isLoadingRecords" class="text-center py-10 text-gray-400">
              <div class="animate-spin w-8 h-8 border-4 border-gray-200 border-t-blue-500 rounded-full mx-auto mb-2"></div>
              기록을 불러오는 중...
            </div>
            
            <div v-else-if="records.length === 0" class="text-center py-12 text-gray-400 bg-gray-50 rounded-xl border border-dashed border-gray-200">
              작성된 수업 기록이 없습니다.
            </div>
            ...
            <div v-else class="space-y-3">
              <div v-for="rec in records" :key="rec.id" class="bg-gray-50 border border-gray-200 p-4 rounded-xl relative group">
                ...
                <p class="text-gray-800 whitespace-pre-wrap leading-relaxed text-sm font-medium">{{ rec.content }}</p>
              </div>
            </div>
          </div>
