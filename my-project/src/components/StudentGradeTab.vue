    ...
    <div class="space-y-4">
      <div v-for="grade in (student.grades || []).slice().reverse()" :key="grade.id" class="bg-gray-50 border border-gray-200 p-5 rounded-xl shadow-sm hover:shadow transition-shadow">
        ...
        <div class="flex flex-wrap gap-3">
          <div v-for="(score, subject) in grade.scores" :key="subject" class="bg-white px-4 py-2 rounded-lg border border-gray-200 shadow-sm flex items-center gap-2 min-w-[80px]">
            <span class="text-xs text-gray-500 font-bold bg-gray-100 px-2 py-1 rounded">{{ subject }}</span>
            <span class="font-black text-indigo-600 text-base">{{ score }}</span>
          </div>
        </div>
